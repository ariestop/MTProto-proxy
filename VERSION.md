# Версионирование проекта mtproxy — **1.2.19**
...
- Сбор клиентов внутри контейнера теперь работает даже без `ss`: добавлен разбор `/proc/net/tcp` через `docker exec` (ESTABLISHED к локальному `:PORT`), чтобы получать реальные IP клиентов в минимальных образах.
...
- Чтение `mtproto_config.txt`: значения теперь очищаются от `CRLF` (`\\r`) и пробелов (важно для `PORT`, иначе парсинг `conntrack` мог давать 0 совпадений при видимых входящих `dport=443`).
...
- Добавлен `MTPROXY_DEBUG=1`: в лог пишется, сколько потоков найдено за цикл (conntrack/ss/docker-ss) и сколько строк записано в `sessions.tsv`.
...
- В опросном сборщике добавлена минимальная запись при первом обнаружении потока (1 сек), чтобы `sessions.tsv` не оставался пустым при коротких сессиях.
- `trap EXIT` больше не «шумит» в подшеллах (ограничен основным `BASHPID`).
...
- При опросе активных потоков длительность теперь добавляется инкрементально на каждом цикле (между предыдущим и текущим `now`), чтобы `sessions.tsv` начинал заполняться сразу, не дожидаясь исчезновения записи `conntrack` (TIME_WAIT может держаться долго).
...
- Для очень коротких потоков (попали только в один снимок опроса) длительность могла получаться 0 и запись отбрасывалась. Теперь длительность округляется минимум до 1 секунды.
...
- Исправлена ложная диагностика «сборщик упал» при запуске `report/status` без `sudo`: если `kill -0` возвращает `EPERM` для root-процесса, теперь делается проверка через `ps -p PID`.
...
- Сбор по опросу: теперь сессия записывается при исчезновении потока из следующего опроса (first_seen → last_seen), чтобы учитывать короткие подключения (TIME_WAIT), которые не всегда попадают в два соседних опроса.
...
- Сбор дополняется `ss` (`state established` к локальному `*:PORT`): при Docker `-p` входящие клиенты часто видны у `docker-proxy`, а в `conntrack` остаётся только исходящий трафик контейнера на внешние `:443`.
- `diagnose`: счётчик и пример строк `ss ESTAB` к `*:PORT`.
- `MTPROXY_NO_SS=1` — отключить опрос `ss`.
...
- Исправлена логика определения IP клиента при сборе через `conntrack -L`: теперь учитываются только входящие соединения к `PORT` на локальные IP хоста (иначе в Docker-режиме часто попадали исходящие соединения прокси к внешним IP:443).
- Добавлено определение локальных IP через `ip -4 addr` (или `MTPROXY_LOCAL_IPS`, или `SERVER=` из конфига как fallback).
...
- Добавлен `trap` в `collect`: при завершении сборщика пишется причина (rc/строка/команда) в `collector.log`, чтобы диагностировать «сборщик упал».
